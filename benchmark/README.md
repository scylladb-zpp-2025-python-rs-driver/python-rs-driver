...
Plot all benchmarks for the `driver-test` backend:
...
Plot all benchmarks for the `concurrent-driver-test` backend:

```sh
sdb -d test.db plot \
  -b config.yml \
  -o concurrent-result.png \
  --series concurrent-driver-test@./ \
  series
```
