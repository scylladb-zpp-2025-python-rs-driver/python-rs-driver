...
use scylla::serialize::writers::CellValueBuilder;
...
use scylla::value::{CqlVarintBorrowed, ValueOverflow};
...
impl<'a, 'py> PyAnyWrapper<'a, 'py> {
    fn python_type_name(&self) -> Result<String, SerializationError> {
        let name = self
            .get_type()
            .name()
    ...
    fn serialize_natives<'b>(
        ...
        match native_type {
            ...
            NativeType::Varint => {
                ...
                // The `to_bytes()` method requires the number of bytes necessary to represent
                // the given integer. The '+1' accounts for values such as 128, where `bits_len / 8 = 1`,
                // but two's-complement representation in big-endian order (CqlVarintBorrowed format)
                // requires 2 bytes, since a single signed byte covers only the range -128..127.
                let bytes_len = bits_len / 8 + 1;

                // TODO: I think there might be a way to create PyDict only once
                // and reuse it in every `Varint` serialization.
                let py_dict = PyDict::new(self.py());
                py_dict.set_item("signed", true).map_err(|e| {
                    SerializationError::new(PythonDriverSerializationError::PythonError(e))
                })?;

                // CqlVarintBorrowed is represented as two's-complement binary in big-endian order
                // and Python's to_bytes() with byteorder='big' and signed=True also returns
                // bytes in this format.
                let bytes = self
                    .call_method("to_bytes", (bytes_len, "big"), Some(&py_dict))
                    .map_err(|_| self.mismatched_type_error::<CqlVarint>(typ))?;
            ...
            NativeType::Decimal => {
                ...
                let decimal = CqlDecimal::try_from(value)
    ...
    // TODO: Update this once PyO3 switches from `FromPyObject` to `FromPyObjectBound`.
    // This will allow `serialize_native` to accept `&str` directly, making `serialize_text` redundant.
    ...
    {
        let Ok(value) = self.extract::<T>() else {
    ...
    fn serialize_text<'b>(
        ...
        let Ok(value) = self.extract::<&str>() else {
            let expected = self.map_type_to_expected()?;
    ...
    fn serialize_arbitrary_value<'b>(
        ...
        match typ {
            ...
            ColumnType::Collection {
                ...
                _ => {
                    let python_collection_name = self.get_type().name().map_err(|e| {
                        SerializationError::new(PythonDriverSerializationError::PythonError(e))
                    })?;

                    let python_collection_name =
                        python_collection_name.extract::<&str>().map_err(|e| {
                            SerializationError::new(PythonDriverSerializationError::PythonError(e))
                        })?;

                    Err(SerializationError::new(
                        PythonDriverSerializationError::UnknownCollectionType(
                            python_collection_name.to_string(),
                        ),
                    ))
                }
            },
...
pub(crate) struct PyMapWrapper<'a, 'py>(&'a Bound<'py, PyMapping>);
...
impl<'py> SerializeValue for PyUdtWrapper<'py, '_> {
    fn serialize<'a>(
        ...
        for (field_name, field_type) in &self.definition.field_types {
            ...
            PyAnyWrapper(&item).serialize_arbitrary_value(field_type, builder.make_sub_writer())?;
        }
