use crate::{routing, utils::GenericPyIterator};
...
impl Sharder {
    ...
    fn iter_source_ports_for_shard(&self, shard: Shard) -> GenericPyIterator {
        GenericPyIterator {
            rust_iter: Box::new(
                self._inner
                    .iter_source_ports_for_shard(shard)
                    .map(|p| Python::attach(|py| (p as usize).into_py_any(py))),
            ),
        }
    }
}
