...
use crate::policies::load_balancing::{PyLoadBalancingPolicy, build_load_balancing_policy};
...
impl PreparedStatement {
    ...
    fn with_load_balancing_policy(&self, policy: Py<PyAny>) -> PreparedStatement {
        ...
        let mut p = self._inner.clone();
        p.set_load_balancing_policy(Some(arc));
        PreparedStatement {
            _inner: p,
            _load_balancing_policy: Some(stored),
...
impl Statement {
    ...
    fn with_load_balancing_policy(&self, policy: Py<PyAny>) -> Statement {
        let (stored, arc) = build_load_balancing_policy(policy);
        let mut s = self._inner.clone();
        s.set_load_balancing_policy(Some(arc));
        Statement {
            _inner: s,
            _load_balancing_policy: Some(stored),
            _execution_profile: self._execution_profile.clone(),
