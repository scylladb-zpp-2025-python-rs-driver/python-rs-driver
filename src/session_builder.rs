...
impl SessionBuilder {
    ...
    fn tls_context<'py>(
        mut slf: PyRefMut<'py, Self>,
        tls_context: PyRef<'py, PyTlsContext>,
    ) -> PyRefMut<'py, Self> {
        slf.config.tls_context = Some(tls_context.inner.clone().into());
        slf
    }

    fn disable_tls(mut slf: PyRefMut<'_, Self>) -> PyRefMut<'_, Self> {
        slf.config.tls_context = None;
        slf
    }
