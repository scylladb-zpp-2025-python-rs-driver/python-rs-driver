...
async def test_basic_serialization(session: Session, table_factory: TableFactory):
    ...
    assert str(result).strip() == "|1|'Test Name'|95.5|"
...
async def test_varint_serialization(session: Session, table_factory: TableFactory):
    ...
    val2 = -123456789012345678901234567890

    await session.execute(f"INSERT INTO {table} (id, col) VALUES (?, ?)", (1, val1))
    await session.execute(f"INSERT INTO {table} (id, col) VALUES (?, ?)", (2, val2))
    await session.execute(f"SELECT * from {table}")
