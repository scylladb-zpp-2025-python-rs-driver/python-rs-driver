...
async def test_prepare_and_execute():
    ...
    assert next(result_str.iter_page())["cluster_name"] == next(result_statement.iter_page())["cluster_name"]
...
async def test_prepare_and_str():
    ...
    cluster_name_str = next(result_str.iter_page())["cluster_name"]
    assert next(result_prepared.iter_page())["cluster_name"] == cluster_name_str
    assert cluster_name_str == next(result_statement.iter_page())["cluster_name"]
