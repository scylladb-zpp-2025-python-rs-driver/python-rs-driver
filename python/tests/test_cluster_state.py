...
async def set_up() -> Session:
    ...
    """)
    # await session.execute(f"INSERT INTO {KEYSPACE}.{TABLE} (id, name) VALUES (42, 'test')")
    return session
...
@pytest.mark.asyncio
@pytest.mark.requires_db
async def test_get_keyspaces_returns_dict(cluster_state: ClusterState) -> None:
...
@pytest.mark.asyncio
@pytest.mark.requires_db
async def test_keyspace_tables_dict(cluster_state: ClusterState) -> None:
    ks = cluster_state.get_keyspace(KEYSPACE)
    assert ks is not None
    assert isinstance(ks.tables, dict)
    assert TABLE in ks.tables


@pytest.mark.asyncio
@pytest.mark.requires_db
async def test_keyspace_views_dict(cluster_state: ClusterState) -> None:
    ks = cluster_state.get_keyspace(KEYSPACE)
    assert ks is not None
    assert isinstance(ks.views, dict)
...
async def test_node_type(cluster_state: ClusterState) -> None:
    assert isinstance(cluster_state.nodes_info, dict)
