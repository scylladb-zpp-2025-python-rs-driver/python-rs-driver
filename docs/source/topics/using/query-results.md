...
By default, each row is represented as a dictionary:
...
Fetching a large result set without paging can put unnecessary pressure on both the driver and the database. If the result is large, or if the query needs to scan many tombstones, an unpaged query may:

- increase latency,
- use much more memory in the driver,
- put higher load on the cluster,
- be more likely to time out.
